...
For full documentation, see README.md in this directory.
    ...
    if valid_preds.sum() > 0:
        ...
        if both_valid.sum() > 0:
            ...
            # ============================================================
            # LAST 9 / 6 MONTHS - same metrics, progressively tighter windows
            # ============================================================
            # Every window ENDS on the same anchor date as the 12-month block
            # (last_date_12m), so the three are directly comparable and the only
            # thing changing is how far back the window reaches. Reading them
            # together shows whether skill is holding up or decaying as you
            # approach today.
            # NOTE: the shorter the window, the larger the share of rows whose
            # forward_return was forward-filled rather than truly realized, so the
            # 6-month figures are the most flattered of the three. Prefer rank IC,
            # and treat the very recent edge with care.
            perf_base = df_predictions.loc[both_valid, ['Date', pred_col, actual_col]].copy()
            perf_base['Date'] = pd.to_datetime(perf_base['Date'])

            for window_months in (9, 6):
                cutoff_win = last_date_12m - pd.DateOffset(months=window_months)
                perf_win = perf_base[perf_base['Date'] > cutoff_win]

                if len(perf_win) == 0:
                    print(f"\n📅 LAST {window_months} MONTHS: no rows with realized outcomes "
                          f"in the trailing {window_months} months")
                    continue

                corr_win = perf_win[pred_col].corr(perf_win[actual_col])
                mae_win = mean_absolute_error(perf_win[actual_col], perf_win[pred_col])
                rmse_win = np.sqrt(mean_squared_error(perf_win[actual_col], perf_win[pred_col]))
                dir_win = ((perf_win[pred_col] > 0) == (perf_win[actual_col] > 0)).mean()

                # Cross-sectional rank IC: mean per-date Spearman (ranking quality,
                # the metric that actually matters for picking stocks)
                ic_by_date_win = perf_win.groupby('Date').apply(
                    lambda g: g[pred_col].corr(g[actual_col], method='spearman')
                    if len(g) >= 20 else np.nan
                ).dropna()

                print(f"\n📅 LAST {window_months} MONTHS ({cutoff_win:%Y-%m-%d} → {last_date_12m:%Y-%m-%d}):")
                print(f"  • Rows with realized outcome: {len(perf_win):,}")
                print(f"  • Correlation with actual returns: {corr_win:.4f} ({corr_win*100:.1f}%)")
                print(f"  • Mean Absolute Error: {mae_win:.2f}%")
                print(f"  • Root Mean Squared Error: {rmse_win:.2f}%")
                print(f"  • Direction accuracy: {dir_win:.2%}")
                if len(ic_by_date_win) > 0:
                    print(f"  • Cross-sectional rank IC (mean daily): {ic_by_date_win.mean():+.4f} "
                          f"[{100*(ic_by_date_win > 0).mean():.0f}% of {len(ic_by_date_win)} days positive]")
