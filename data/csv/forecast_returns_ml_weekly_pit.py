...
class ReturnForecaster:
    ...
    def __init__(self, lookback_days=10, forecast_days=10, target_return_days=None,
                 ...
                 emulate_prod_ffill=False, walk_frequency='monthly', train_start=None,
                 target_residual='none', composite_target=None):
        """
        ...
        # POINT-IN-TIME GUARD (fresh-run realism):
        # When True, a month's model may only train on rows whose forward-return target
        # was ALREADY realized before the prediction month started. This prevents the
        # forward-looking 90-day target from leaking future outcomes into recent-month
        # training when rebuilding from a latest-snapshot CSV. See create_target() and
        # _walk_forward_predict() for where target_date is captured and enforced.
        self.point_in_time = point_in_time
        # Optional feature whitelist: restrict the model to a specific set of features
        # (used to test whether ~15-20 top features match the full ~265-feature model).
        self.feature_whitelist = set(feature_whitelist) if feature_whitelist else None
        # If True, drop ALL fundamental features and keep only price/volume-derived
        # (technical) features + sector labels — for a purely technical, fast short-horizon model.
        self.technicals_only = technicals_only
        # If True, drop ONLY the new t_-prefixed technical batch; keep the earlier price
        # features (return_/volatility_/momentum_/zscore/volume) and all fundamentals.
        self.fundamental_only = fundamental_only
        # If True, MASK-AND-FFILL the target per month during the walk-forward: keep ALL
        # training rows, but rows whose target isn't known yet as-of month M get the stock's
        ...
        # WEEKLY WALK: 'W-MON' periods run Tuesday->Monday, so period.to_timestamp()
        # IS the Tuesday. Every downstream use (to_timestamp, end_time, period-1) is
        # identical to the monthly path -- only the freq string changes.
        self.walk_frequency = walk_frequency
        self.walk_freq = 'W-MON' if walk_frequency == 'weekly' else 'M'
        self.train_start = pd.Timestamp(train_start) if train_start else None
        self.target_residual = target_residual
        self.composite_target = composite_target   # list of (horizon, weight)
    ...
    def create_target(self, df):
        ...
        # POINT-IN-TIME GUARD: capture the calendar date on which this row's target
        # actually completes (the date of the price used for price_at_target). A row's
        # forward_return is only KNOWABLE once this date has passed. We keep this column
        # so the walk-forward loop can exclude training rows whose target had not yet been
        # realized as of the prediction month. NaT for rows without enough future data.
        ...
        # ===== COMPOSITE MULTI-HORIZON TARGET (optional) =====
        # Blend horizons, e.g. 50%x90d + 35%x20d + 10%x10d + 5%x5d. The long leg
        # carries the signal; the short legs stay realizable at the data edge, so
        # recent rows keep a genuine (not ffilled) target. Each leg's own realization
        # date is stored so the walk-forward can rebuild the blend per period from
        # only the legs that have actually realized -- keeping it point-in-time.
        if self.composite_target:
            print('  ⚗️  Composite target: ' +
                  ' + '.join(f'{w:.0%}x{h}d' for h, w in self.composite_target))
            gp = df.groupby('Symbol', sort=False)
            p_fc = gp['RefPriceClose'].shift(-self.forecast_days)
            self._comp_cols = []
            for h, w in self.composite_target:
                r = (gp['RefPriceClose'].shift(-(self.forecast_days + h)) / p_fc - 1) * 100
                r = r.where((r <= 500) & (r >= -99))
                df[f'_cmp_r{h}'] = r
                df[f'_cmp_td{h}'] = gp['Date'].shift(-(self.forecast_days + h))
                self._comp_cols.append((h, w))
                print(f'       {h:>3}d component: {r.notna().sum():>10,} rows realizable')
            # fillna(0) on the NUMERATOR is essential: a plain sum propagates NaN and
            # would blank the target wherever the longest leg is missing -- exactly the
            # edge rows this feature exists to rescue (they would then be dropped by
            # valid_idx before the walk-forward ever saw them).
            num = sum(df[f'_cmp_r{h}'].fillna(0) * w for h, w in self._comp_cols)
            den = sum(df[f'_cmp_r{h}'].notna() * w for h, w in self._comp_cols)
            df['forward_return'] = (num / den.replace(0, np.nan))
            print(f"       blended target: {df['forward_return'].notna().sum():,} rows")

        # ===== RESIDUAL / RISK-ADJUSTED TARGET (optional) =====
        # 'beta' removes the market leg (measured: beta-neutral earned +46bps/10d vs
        # -1bp naked). 'vol' divides by trailing sigma (smallest mcap decile otherwise
        # carries ~81% of squared error). Risk estimates use forecast_days-LAGGED
        # trailing returns only, so they are knowable at decision time (verified: a
        # scramble of all future prices leaves pre-cutoff targets bit-identical).
        if self.target_residual and self.target_residual != 'none':
            df = df.sort_values(['Symbol', 'Date'], kind='stable').reset_index(drop=True)
            g2 = df.groupby('Symbol', sort=False)
            r1 = g2['RefPriceClose'].pct_change() * 100
            r1_lag = r1.groupby(df['Symbol'], sort=False).shift(self.forecast_days)
            win = max(60, self.target_return_days)
            mp = max(20, win // 3)
            sigma = (r1_lag.groupby(df['Symbol'], sort=False)
                           .rolling(win, min_periods=mp).std().reset_index(level=0, drop=True))
            resid_sigma = None
            if self.target_residual in ('beta', 'both'):
                mkt = r1_lag.groupby(df['Date'], sort=False).transform('mean')
                roll = lambda x: (x.groupby(df['Symbol'], sort=False)
                                   .rolling(win, min_periods=mp).mean()
                                   .reset_index(level=0, drop=True))
                cov = roll(r1_lag.mul(mkt)) - roll(r1_lag) * roll(mkt)
                mvar = (mkt.groupby(df['Symbol'], sort=False)
                           .rolling(win, min_periods=mp).var().reset_index(level=0, drop=True))
                beta = (cov / mvar.replace(0, np.nan)).clip(-3, 3)
                fwd_mkt = df.groupby('Date', sort=False)['forward_return'].transform('mean')
                df['forward_return'] = df['forward_return'] - beta * fwd_mkt
                # RESIDUAL vol for 'both': dividing a beta-stripped numerator by TOTAL
                # vol over-shrinks high-beta names (measured: cohort dispersion ratio
                # 2.77x, WORSE than no scaling at all; residual vol gives 0.97x).
                resid_daily = r1_lag - beta * mkt
                resid_sigma = (resid_daily.groupby(df['Symbol'], sort=False)
                                          .rolling(win, min_periods=mp).std()
                                          .reset_index(level=0, drop=True))
                print(f'  ✓ Target: beta-adjusted (rolling {win}d beta, clipped +/-3)')
            if self.target_residual in ('vol', 'both'):
                use = resid_sigma if resid_sigma is not None else sigma
                df['forward_return'] = df['forward_return'] / (use * np.sqrt(
                    self.target_return_days)).replace(0, np.nan)
                kind = 'residual' if resid_sigma is not None else 'total'
                print(f'  ✓ Target: volatility-scaled ({kind} {win}d sigma)')
            df['forward_return'] = df['forward_return'].replace([np.inf, -np.inf], np.nan)
            print(f"    Target rows retained: {df['forward_return'].notna().sum():,} "
                  f"(units are NO LONGER percent)")
    ...
    def engineer_features(self, df):
        ...
        # ===== Medium-term MOMENTUM features (30/60/90-day trailing returns) =====
        # EXPERIMENT: added to test whether genuine price momentum is predictive under a
        # properly point-in-time-guarded label. If the old leaky label was manufacturing a
        # fake momentum effect, explicit momentum here will show whether real momentum
        # survives once the leakage is gone. Uses LAGGED price (T-1) => NO look-ahead.
        print("  • Creating 30/60/90-day momentum features...")
        for days in [30, 60, 90]:
            df[f'momentum_{days}d'] = df.groupby('Symbol')[price_col].pct_change(days) * 100

        # ===== 90-day momentum, cross-sectionally Z-SCORED per date (pipeline-style factor) =====
        # Standardizes 90d momentum across all stocks WITHIN each date (like a Zipline
        # pipeline .zscore()): z = (mom90 - cross-sectional mean) / cross-sectional std.
        # Within-date only => NO look-ahead; built from lagged price. Making momentum
        # regime-relative (vs its raw magnitude, which drifts across vol regimes) tends to
        # be more stable than raw momentum. Clipped to +/-5 to tame outliers.
        print("  • Creating cross-sectional z-scored 90d momentum factor...")
        _gd = df.groupby('Date')['momentum_90d']
        df['momentum_90d_zscore'] = (
            (df['momentum_90d'] - _gd.transform('mean')) / (_gd.transform('std') + 1e-8)
        ).clip(-5, 5)
        ...
        # ==================================================================
        # TECHNICAL INDICATORS (price + volume ONLY) — short-horizon trend / sector
        # rotation / downside-vol forecasting. ALL built from LAGGED price/volume
        # (RefPriceClose_lag1 / RefVolume_lag1 = T-1) with strictly BACKWARD windows
        # (pct_change / rolling / ewm / cumsum) and NO forward shift  =>  ZERO look-ahead.
        # No OHLC in the data, so rolling max/min of close proxies high/low.
        # Every feature here is prefixed 't_' so --technicals-only can select the subset.
        # ==================================================================
        print("  • Creating technical indicators (price/volume, zero look-ahead)...")
        _sym = df['Symbol']
        P = df[price_col]                     # lagged close (T-1)
        Vv = df[volume_col]                   # lagged volume (T-1)
        r1 = df.groupby('Symbol')[price_col].pct_change(1)   # daily return (backward)
        df['t_ret1'] = r1 * 100

        def _roll(s, n, fn, mp=None):
            """Backward rolling stat of series s, per symbol (no look-ahead)."""
            mp = mp if mp is not None else max(2, n // 2)
            return s.groupby(_sym).transform(lambda x: getattr(x.rolling(n, min_periods=mp), fn)())

        # --- Multi-horizon momentum (short -> long) ---
        for n in [3, 5, 10, 15, 21, 42, 63, 126, 252]:
            df[f't_ret_{n}'] = df.groupby('Symbol')[price_col].pct_change(n) * 100

        # --- Moving-average trend location & crossovers ---
        _sma = {}
        for n in [5, 10, 20, 50, 100, 200]:
            _sma[n] = _roll(P, n, 'mean')
            df[f't_px_to_sma{n}'] = (P / _sma[n] - 1) * 100
        df['t_sma5_20']   = (_sma[5]  / _sma[20]  - 1) * 100
        df['t_sma20_50']  = (_sma[20] / _sma[50]  - 1) * 100
        df['t_sma50_200'] = (_sma[50] / _sma[200] - 1) * 100   # golden/death-cross proxy

        # --- EMAs & MACD (fast trend) ---
        _e12 = P.groupby(_sym).transform(lambda x: x.ewm(span=12, adjust=False).mean())
        _e26 = P.groupby(_sym).transform(lambda x: x.ewm(span=26, adjust=False).mean())
        _macd = _e12 - _e26
        _sig  = _macd.groupby(_sym).transform(lambda x: x.ewm(span=9, adjust=False).mean())
        df['t_macd']      = (_macd / P) * 100
        df['t_macd_hist'] = ((_macd - _sig) / P) * 100

        # --- RSI (momentum oscillator) ---
        _gain = r1.clip(lower=0); _loss = (-r1).clip(lower=0)
        for n in [7, 14, 21]:
            _ag = _roll(_gain, n, 'mean'); _al = _roll(_loss, n, 'mean')
            df[f't_rsi{n}'] = 100 - 100 / (1 + _ag / (_al + 1e-9))

        # --- Stochastic %K / Williams %R / range position (close-proxy high/low) ---
        for n in [14, 20]:
            _hi = _roll(P, n, 'max'); _lo = _roll(P, n, 'min'); _rng = (_hi - _lo).replace(0, np.nan)
            df[f't_stoch{n}']    = ((P - _lo) / _rng) * 100
            df[f't_willr{n}']    = ((_hi - P) / _rng) * -100
            df[f't_rangepos{n}'] = (P - _lo) / _rng

        # --- Bollinger %B & band width (vol regime) ---
        for n in [20, 50]:
            _ma = _roll(P, n, 'mean'); _sd = _roll(P, n, 'std')
            df[f't_bbpos{n}']   = (P - _ma) / (2 * _sd + 1e-9)
            df[f't_bbwidth{n}'] = (4 * _sd) / (_ma + 1e-9)

        # --- Volatility: TOTAL + DOWNSIDE (semi-deviation) + skew ---
        _neg = r1.where(r1 < 0)
        for n in [5, 10, 21, 42, 63]:
            df[f't_vol{n}']     = _roll(r1,   n, 'std') * np.sqrt(252) * 100
            df[f't_downvol{n}'] = _roll(_neg, n, 'std') * np.sqrt(252) * 100
        df['t_vol_ratio']  = df['t_vol10']    / (df['t_vol63'] + 1e-9)   # short/long vol (regime shift)
        df['t_down_ratio'] = df['t_downvol21'] / (df['t_vol21'] + 1e-9)  # downside share of total vol
        for n in [21, 63]:
            df[f't_skew{n}'] = _roll(r1, n, 'skew', mp=max(5, n // 3))

        # --- Drawdown from rolling high (downside risk) & ATR proxy ---
        for n in [21, 63, 126]:
            df[f't_drawdown{n}'] = (P / _roll(P, n, 'max') - 1) * 100    # <= 0
        _tr = r1.abs()
        for n in [14, 21]:
            df[f't_atr{n}'] = _roll(_tr, n, 'mean') * 100

        # --- Trend acceleration / slope / persistence (reactivity to trend change) ---
        df['t_accel_5_21']  = df['t_ret_5']  - df['t_ret_21']
        df['t_accel_10_42'] = df['t_ret_10'] - df['t_ret_42']
        for n in [20, 50]:
            df[f't_slope{n}'] = (df.groupby('Symbol')[price_col].pct_change(n) / n) * 100
        _up = (r1 > 0).astype(float)
        for n in [10, 21]:
            df[f't_updays{n}'] = _roll(_up, n, 'mean')

        # --- Volume / money-flow ---
        for n in [5, 10, 20, 50]:
            df[f't_volsurge{n}'] = Vv / (_roll(Vv, n, 'mean') + 1e-9)
        df['t_dollar_vol'] = np.log1p((P * Vv).clip(lower=0))
        _sv = np.sign(r1).fillna(0) * Vv
        for n in [10, 21]:
            df[f't_netflow{n}'] = _roll(_sv, n, 'sum') / (_roll(Vv, n, 'sum') + 1e-9)   # net buying pressure [-1,1]
        _mf = P * Vv
        _pmf = _roll(_mf.where(r1 > 0, 0.0), 14, 'sum'); _nmf = _roll(_mf.where(r1 < 0, 0.0), 14, 'sum')
        df['t_mfi14'] = 100 - 100 / (1 + _pmf / (_nmf + 1e-9))

        # --- Cross-sectional & SECTOR-relative strength (rotation; within-date => no look-ahead) ---
        for n in [21, 63]:
            df[f't_relstr{n}'] = df[f't_ret_{n}'] - df.groupby('Date')[f't_ret_{n}'].transform('mean')
        if 'GICSSectorName' in df.columns:
            for n in [21, 63]:
                df[f't_sectrel{n}'] = df[f't_ret_{n}'] - df.groupby(['Date', 'GICSSectorName'])[f't_ret_{n}'].transform('mean')

        # keep finite; leading-window NaNs handled by the forward-fill/fillna step below
        _tcols = [c for c in df.columns if c.startswith('t_')]
        df[_tcols] = df[_tcols].replace([np.inf, -np.inf], np.nan)
        print(f"  ✓ Added {len(_tcols)} technical indicators (all backward-looking, zero look-ahead)")
        ...
        # CRITICAL: never forward-fill / zero-fill the TARGET. forward_return must stay NaN
        # where the future outcome doesn't exist yet, otherwise valid_idx (= ~isnan(target))
        # is defeated (becomes all-True), the model trains on fake/stale targets, and the
        # script's own diagnostics/IC get leakage-inflated. Keeping it NaN restores valid_idx
        # as the point-in-time guard and makes the stored forward_return honest at the edge.
        # ALWAYS protect the target from the global ffill (keeps forward_return NaN where
    ...
    def prepare_features(self, df):
        ...
        # OPTIONAL: restrict to a feature whitelist (the "fewer factors, same alpha" test).
        # df keeps ALL engineered columns (so per-month rank computation still reads its base
        # columns); we only restrict X / feature_cols / _rank_cols to the requested set.
        if self.feature_whitelist:
            wl = self.feature_whitelist
            kept = [c for c in feature_cols if c in wl]
            missing = [c for c in wl if c not in feature_cols]
            X = X[kept].copy()
            feature_cols = kept
            if hasattr(self, '_rank_cols') and self._rank_cols:
                # keep only base cols whose _rank survived the whitelist
                self._rank_cols = [c for c in self._rank_cols if f'{c}_rank' in wl]
            print(f"  • 🎯 FEATURE WHITELIST: using {len(kept)} of {len(wl)} requested features")
            if missing:
                print(f"    ⚠️  {len(missing)} requested features not found (skipped): {sorted(missing)[:10]}")

        # OPTIONAL: technicals-only — drop all fundamentals, keep price/volume-derived
        # features (+ sector labels for rotation context). df keeps all columns so the
        # per-month rank computation still reads its base columns.
        if self.technicals_only:
            tech_prefixes = ('t_', 'return_', 'volatility_', 'momentum_', 'volume_',
                             'RefPriceClose', 'RefVolume', 'CompanyMarketCap')
            sector_ctx = {'GICSSectorName', 'sharadar_sicsector', 'sharadar_sicindustry'}
            kept = [c for c in feature_cols if str(c).startswith(tech_prefixes) or c in sector_ctx]
            dropped = len(feature_cols) - len(kept)
            X = X[kept].copy()
            feature_cols = kept
            if hasattr(self, '_rank_cols') and self._rank_cols:
                self._rank_cols = [c for c in self._rank_cols
                                   if str(c).startswith(('t_', 'return_', 'volatility_')) or 'MarketCap' in str(c)]
            print(f"  • 🔧 TECHNICALS-ONLY: kept {len(kept)} price/volume features, dropped {dropped} fundamentals")

        # OPTIONAL: fundamental-only — drop ONLY the new t_-prefixed technical batch,
        # keep the earlier price features (return_/volatility_/momentum_/zscore/volume)
        # and all fundamentals. (df keeps all columns for per-month rank computation.)
        if self.fundamental_only:
            kept = [c for c in feature_cols if not str(c).startswith('t_')]
            dropped = len(feature_cols) - len(kept)
            X = X[kept].copy()
            feature_cols = kept
            if hasattr(self, '_rank_cols') and self._rank_cols:
                self._rank_cols = [c for c in self._rank_cols if not str(c).startswith('t_')]
            print(f"  • 🧾 FUNDAMENTAL-ONLY: dropped {dropped} new t_ technical factors, kept {len(kept)} features")
    ...
    def _walk_forward_predict(self, df, X, y, sample_weights, valid_idx,
        ...
        df['_year_month'] = df['Date'].dt.to_period(self.walk_freq)

        # Get unique year-months sorted
        unique_months = sorted(df['_year_month'].unique())

        print(f"\n📅 Date Range:")
        _unit = "week" if self.walk_frequency == "weekly" else "month"
        print(f"  • Walk frequency: {self.walk_frequency}")
        print(f"  • First {_unit}: {unique_months[0]}")
        print(f"  • Last {_unit}: {unique_months[-1]}")
        print(f"  • Total {_unit}s: {len(unique_months)}")
        ...
        # --train-start: hold the leading span back as pure lookback. Those rows
        # still TRAIN (they precede the first prediction period) but are never predicted.
        if self.train_start is not None:
            before = len(months_to_process)
            months_to_process = [m for m in months_to_process
                                 if m.to_timestamp() >= self.train_start]
            print(f"  • --train-start {self.train_start.date()}: predicting "
                  f"{len(months_to_process)} of {before} periods "
                  f"({before - len(months_to_process)} held out as lookback)")
        ...
        for i, current_month in enumerate(unique_months, 1):
            ...
            # NOTE (POINT-IN-TIME): the target-maturity guard is applied LATER, at the
            # final X_train/y_train selection (search "PIT training-row filter"), NOT here.
            # train_positions must stay the FULL cross-section so the downstream
            # cross-sectional RANKING computation (all_positions = train ∪ predict) uses
            # every stock present at each date. Trimming it here would compute percentile
            # ranks over a maturity-biased subset and corrupt rank features for ALL months.
            ...
            current_month_period = pd.Period(current_month, freq=self.walk_freq)
            ...
            # ================================================================
            # PIT training-row filter (the core point-in-time guard)
            # ================================================================
            # Rankings above used the FULL cross-section (train_positions ∪ predict) so
            # percentiles are correct. NOW — and only now — restrict the rows the model
            # actually LEARNS from to those whose forward-return target had already been
            # realized before this month. This removes the forward-target leakage at the
            # data edge without touching the cross-sectional ranking universe.
            # NaT compares False, so rows without enough future data are excluded too.
            train_positions_fit = train_positions
            y_fit = None

            if self.composite_target and getattr(self, '_comp_cols', None):
                # Rebuild the blend from ONLY the legs realized before this period,
                # renormalising weights over the survivors. A row near the data edge
                # is scored on its short legs rather than dropped or ffilled.
                fdm = np.datetime64(first_day_of_month)
                num = den = None
                avail = {}
                for h, w in self._comp_cols:
                    ok_h = (df[f'_cmp_td{h}'].values < fdm) & df[f'_cmp_r{h}'].notna().values
                    avail[h] = int(ok_h[train_positions].sum())
                    contrib = np.where(ok_h, df[f'_cmp_r{h}'].values * w, 0.0)
                    wt = np.where(ok_h, w, 0.0)
                    num = contrib if num is None else num + contrib
                    den = wt if den is None else den + wt
                y_comp = np.divide(num, den, out=np.full(len(df), np.nan), where=den > 0)
                fit_all = y_comp[train_positions]
                ok = ~np.isnan(fit_all)
                train_positions_fit = train_positions[ok]
                y_fit = fit_all[ok]
                print(f"    ⚗️  composite as-of {first_day_of_month.date()}: "
                      f"{len(train_positions_fit):,} rows  "
                      f"[{' '.join(f'{h}d:' + format(avail[h], ',') for h, _ in self._comp_cols)}]")
                if len(train_positions_fit) < 2000:
                    print(f"  [{i:3d}/{len(unique_months)}] {current_month}: SKIPPED "
                          f"(composite left {len(train_positions_fit):,} < 2,000)")
                    continue

            elif self.ffill_target and 'target_date' in df.columns:
                # MASK-AND-FFILL, as-of month M (matches the live "keep all X, ffill y" process):
                # for training rows whose target completes ON/AFTER M (not knowable at M), we
                # do NOT drop them — we replace y with the stock's LAST-KNOWN realized target
                # (forward-fill). Only past outcomes are ever used => NON-LEAKY, done per month.
                fdm = np.datetime64(first_day_of_month)
                y_masked = pd.Series(y, index=df.index).mask(df['target_date'].values >= fdm)
                y_asof = y_masked.groupby(df['Symbol'], sort=False).ffill()   # last-known per symbol
                fit_y_all = y_asof.iloc[train_positions].values
                ok = ~np.isnan(fit_y_all)          # drop symbols with no known target yet
                train_positions_fit = train_positions[ok]
                y_fit = fit_y_all[ok]
                n_stale = int((df['target_date'].iloc[train_positions_fit].values >= fdm).sum())
                print(f"    ⏩ ffill-target: training on {len(train_positions_fit):,} rows "
                      f"({n_stale:,} carry a stale/ffilled target as-of {first_day_of_month.date()})")
                if len(train_positions_fit) < 2000:
                    print(f"  [{i:3d}/{len(unique_months)}] {current_month}: ⏭️  SKIPPED "
                          f"(ffill-target left insufficient training data: {len(train_positions_fit):,} < 2,000)")
                    continue
            elif self.point_in_time and 'target_date' in df.columns:
                target_dates_train = df.iloc[train_positions]['target_date'].values
                realized_mask = target_dates_train < np.datetime64(first_day_of_month)
                train_positions_fit = train_positions[realized_mask]
                n_dropped = len(train_positions) - len(train_positions_fit)
                if n_dropped > 0:
                    print(f"    🕰️  PIT: training on {len(train_positions_fit):,} rows "
                          f"(dropped {n_dropped:,} whose target realizes on/after {first_day_of_month.date()})")
                if len(train_positions_fit) < 2000:
                    print(f"  [{i:3d}/{len(unique_months)}] {current_month}: ⏭️  SKIPPED "
...
def _parse_composite(spec):
    """'90:0.5,20:0.35' -> [(90,0.5),(20,0.35)], longest horizon first, weights summing to 1."""
    if not spec:
        return None
    out = [(int(h), float(w)) for h, w in (part.split(':') for part in spec.split(','))]
    tot = sum(w for _, w in out)
    if abs(tot - 1.0) > 1e-6:
        out = [(h, w / tot) for h, w in out]
...
For full documentation, see README.md in this directory.
    ...
    parser.add_argument('--feature-whitelist', type=str, default=None,
                        help='Comma-separated list of feature names to restrict the model to '
                             '(e.g. the top ~15-20 features). Tests whether a small feature set '
                             'matches the full ~265-feature model. Ranking features (name ending '
                             '_rank) and their base columns are handled automatically.')
    parser.add_argument('--ffill-target', action='store_true',
                        help='MASK-AND-FFILL the target per month during the walk-forward: keep ALL '
                             'training rows, but rows whose outcome is not yet known as-of month M get '
    ...
    parser.add_argument('--walk-frequency', choices=['monthly', 'weekly'], default='monthly',
                        help='weekly walks Tuesday-anchored weeks (Tue->Mon): train on '
                             'everything strictly before each Tuesday, then forecast that week.')
    parser.add_argument('--train-start', metavar='YYYY-MM-DD',
                        help='Only PREDICT periods starting on/after this date; earlier rows '
                             'still train (reserves a warm-up span).')
    parser.add_argument('--composite-target', metavar='SPEC',
                        help='Blend horizons, e.g. "90:0.5,20:0.35,10:0.10,5:0.05". Rebuilt '
                             'per period from realized legs only, weights renormalised.')
    parser.add_argument('--target-residual', choices=['none','beta','vol','both'], default='none',
                        help="Strip market beta and/or scale by trailing vol from the TARGET. "
                             "Predictions come out in risk units, not percent.")
    ...
    parser.add_argument('--fundamental-only', action='store_true',
                        help='Drop ONLY the newly-added t_ technical batch; keep the earlier '
                             ...
                             'fundamentals. Use to test the model as it was before the big technical set.')
    parser.add_argument('--technicals-only', action='store_true',
                        help='Use ONLY price/volume-derived technical indicators (+ sector labels) '
                             'and drop all fundamental features. For a fast, purely technical '
                             'short-horizon (2-4 week) trend/rotation/downside-vol model.')
    parser.add_argument('--point-in-time', action='store_true',
                        help='TRUE POINT-IN-TIME training. Only train each month on rows whose '
                             'forward-return target had ALREADY been realized before that month. '
                             'Eliminates forward-target leakage at the data edge so a fresh, '
                             'from-scratch run yields a realistic, tradeable signal profile that '
                             'matches point-in-time (weekly) behavior. RECOMMENDED for judging '
                             'backtest performance and for anything you intend to trade.')
    ...
    forecaster = ReturnForecaster(
        ...
        walk_frequency=args.walk_frequency,
        train_start=args.train_start,
        target_residual=args.target_residual,
        composite_target=_parse_composite(args.composite_target)
    )
    if args.ffill_target and args.point_in_time:
        print("\n⚠️  --ffill-target and --point-in-time both set: using --ffill-target "
              "(per-month mask-and-ffill); --point-in-time is ignored.")
    if args.point_in_time:
        print("\n🕰️  POINT-IN-TIME MODE ENABLED")
        print("  • Each month trains ONLY on rows whose forward-return target was realized before that month")
        print("  • Eliminates forward-target leakage at the data edge (realistic, tradeable signal)")
