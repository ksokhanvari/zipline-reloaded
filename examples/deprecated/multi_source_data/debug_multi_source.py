...
def make_debug_pipeline():
    ...
    NOTE: Sharadar ROE is empty in the current bundle, so we use LSEG ROE.
    """
    # Sharadar (no ROE available)
    ...
    # Sharadar filter (using FCF and PE, no ROE)
    sharadar_quality = (s_fcf > 0) & (s_pe > 0) & (s_pe < 30)

    # LSEG filter (using ROE and PEG)
...
try:
    ...
    if len(result) > 0:
        ...
        print(f"  Combined quality: {result['combined_quality'].sum()} stocks")

        if result['combined_quality'].sum() > 0:
            print(f"\n✓ Found {result['combined_quality'].sum()} stocks with combined quality!")
            print("\nTop 5 by LSEG ROE with combined quality:")
            confirmed = result[result['combined_quality']]
            top5 = confirmed.nlargest(5, 'l_roe')
            print(top5[['l_roe', 's_fcf', 's_pe', 'l_peg']].to_string())
        else:
            print("\n⚠️  No stocks found with combined quality!")
            ...
            if result['sharadar_quality'].sum() > 0:
                ...
                print(sharadar_pass[['l_roe', 's_fcf', 's_pe', 'l_peg']].to_string())
            ...
            if result['lseg_quality'].sum() > 0:
                ...
                print(lseg_pass[['l_roe', 's_fcf', 's_pe', 'l_peg']].to_string())
    else:
